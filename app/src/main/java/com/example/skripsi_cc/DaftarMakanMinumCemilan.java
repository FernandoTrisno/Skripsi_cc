...
public class DaftarMakanMinumCemilan extends AppCompatActivity {
    SearchView cari;
        ...
        btn_catatmakanan.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                for(int i=0 ; i<idnama.size();i++){
                    ...
                    db.collection(emailuser).document(tanggal).collection("Makan Cemilan").document(idnama.get(i)).set(checklist, SetOptions.merge());
                    db.collection(emailuser).document(tanggal).collection("Total_Kalori").document("MC"+idnama.get(i)).set(total,SetOptions.merge());
                    db.collection(emailuser).document(tanggal).collection("Total_MC").document("MC"+idnama.get(i)).set(total,SetOptions.merge());
                }
                startActivity(new Intent(DaftarMakanMinumCemilan.this, DetailMakanCemilan.class));
            }
        });
