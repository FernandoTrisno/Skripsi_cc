...
public class DaftarMakanMinumSiang extends AppCompatActivity {
    SearchView cari;
        ...
        btn_catatmakanan.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                for(int i=0 ; i<idnama.size();i++){
                    ...
                    db.collection(emailuser).document(tanggal).collection("Makan Siang").document(idnama.get(i)).set(checklist, SetOptions.merge());
                    db.collection(emailuser).document(tanggal).collection("Total_Kalori").document("MS"+idnama.get(i)).set(total,SetOptions.merge());
                    db.collection(emailuser).document(tanggal).collection("Total_MS").document("MS"+idnama.get(i)).set(total,SetOptions.merge());
                }
                startActivity(new Intent(DaftarMakanMinumSiang.this, DetailMakanSiang.class));
            }
        });
